...
SideEffects: none
Globals Used: none
Notes: currently this has not method to space the events properly.
========================================================*/

//BUG: for some reason not allocating all the events
//TODO: current implementation SUPER SLOW, maybe need to redo?
/*
function evenDistribution (freetimeInDay, eventsInDay){
    ...
    //TODO: what happens when their isnt enough time to space all the events out?
    //     i'd imagine i have to loop the entire for loop with a while loop where i change the beginning
    //     spacing till its zero.
    ...
    for(i = 0; i < amtOfBlocks; i++){
        ...
        var starttime = new Date(totalFreeTime[j].startTime);
        var endtime = new Date(totalFreeTime[j].endTime);

        var freeTimeInBlock = endtime.getTime() - starttime.getTime();

        if (freeTimeInBlock > (timeForEventBlock + (spacing * freeTimeInBlock))) {
            var newEvent = feeds.createGapEvent("something else", starttime, timeForEventBlock);
            newEventsInDay.push(newEvent);
            totalFreeTime  = [totalFreeTime];
            var eventWithGaps = feeds.createGapEvent("something else", starttime, timeForEventBlock + (spacing * freeTimeInBlock))
            removeFreetime(totalFreeTime, 0, j, eventWithGaps);
        ...
        // j itself changing. this potentially result in skips to time block
        var prev_length = timeBlocks.length;
        timeBlocks = totalFreeTime.length;
        if (timeBlocks == prev_length) {
            j++;
        }
    }
...
function assignEventsToDay(freetimeInDay, eventsInDay) {
    ...
    var i;
    for (i = 0; i < amtOfBlocks; i++) {
        var newEvent = eventsInDay[i];
        var difference = (new Date(newEvent.end.dateTime)).getTime() - (new Date(newEvent.start.dateTime)).getTime();
    ...
    for (i = 0; i < timeBlocks; i++) {
        var starttime = new Date(totalFreeTime[i].startTime);
        var endtime = new Date(totalFreeTime[i].endTime);

        var freeTimeInBlock = endtime.getTime() - starttime.getTime();

        free_time += freeTimeInBlock;
    }

    console.log("total free time: ", free_time / 3.6e+6);
    console.log("total event time:", event_time / 3.6e+6);

    //substract freetime by event time
    var remainder_time = free_time - event_time;

    console.log("remainder time: ", remainder_time / 3.6e+6);

    //divide remainder by amount of events, and make it the spacing for events
    var spacing = remainder_time / amtOfBlocks;

    console.log("gaps of this day", spacing / 3.6e+6);
    ...
    for (i = 0; i < amtOfBlocks; i++) {
        ...
        if (freeTimeInBlock >= (timeForEventBlock + spacing)) {
            console.log("event #", i, "was added");
            console.log("freetime of block: ", freeTimeInBlock);
            console.log("time the event requires: ", (timeForEventBlock + spacing));

            var newEvent = feeds.createGapEvent("something else", starttime, timeForEventBlock);
            newEventsInDay.push(newEvent);
            totalFreeTime  = [totalFreeTime];
            var eventWithGaps = feeds.createGapEvent("something else", starttime, timeForEventBlock + spacing);
            removeFreetime(totalFreeTime, 0, j, eventWithGaps);
            totalFreeTime = totalFreeTime[0];
        }
        else {
            if (freeTimeInBlock >= (timeForEventBlock)) {
                var newEvent = feeds.createGapEvent("something else", starttime, timeForEventBlock);
                newEventsInDay.push(newEvent);
                totalFreeTime  = [totalFreeTime];
                var eventWithGaps = feeds.createGapEvent("something else", starttime, timeForEventBlock);
                removeFreetime(totalFreeTime, 0, j, eventWithGaps);
                totalFreeTime = totalFreeTime[0];
            }
        }
