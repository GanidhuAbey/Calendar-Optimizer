...
function assignEventsToDay(freetimeInDay, eventsInDay) {
    var timeForEventBlock = 30 * 60000;
    var timeBlocks = freetimeInDay.length;

    var amtOfBlocks = eventsInDay.length;
    var averageAssignment = amtOfBlocks / timeBlocks;

    var totalFreeTime = freetimeInDay;

    //convert time to 30 minute blocks in freetime
    var eventAssignments = [];
    var i;
    for (i = 0; i < timeBlocks; i++) {
        var timeInBlock = (new Date(totalFreeTime[i].endTime)).getTime() -
                          (new Date(totalFreeTime[i].startTime)).getTime();

        var maximum_assignments = timeInBlock / timeForEventBlock;
        //var assignment_amount = maximum_assignments / averageAssignment;

        eventAssignments.push(maximum_assignments);
    }
    var avg = calculateAverage(eventAssignments);
    var percentageAssignments = [];
    for (i = 0; i < eventAssignments.length; i++) {
        var percentage = eventAssignments[i] / avg;
        percentageAssignments.push(percentage * averageAssignment);
    }

    console.log("possible assignments: ", percentageAssignments);
